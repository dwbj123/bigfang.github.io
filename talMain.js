...
window.TalMain = {
    init: function () {
        ...
        if($('#content').hasClass('article_show_mini_css')){
            _ts.articleMini();
        }
    ...
    //mini 文章
    articleMini: function(){
        var _ts = this,
            ratio = 1.2,
            $content = $('#content'),
            contentH = parseInt($content.height(), 10),
            miniH = parseInt(_ts.config.winH*ratio, 10),
            miniHtml = '<div class="content-minit"><span class="show_btn">展开全文</span><span class="show_more"></span></div>';
        if(contentH > miniH){
            $content.parent('div').addClass('content-box-mini');
            $content.addClass('mini').css({
                'max-height': miniH+'px'
            });
            $content.after(miniHtml);
            $('.content-minit').on('click', function(){
                $(this).hide();
                $content.removeClass('mini');
                $content.css({
                    'max-height': 'none'
                });
                $content.parent('div').removeClass('content-box-mini');
            });
        }
